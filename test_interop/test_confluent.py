#!/usr/bin/env python3
"""
Interoperability test for Bento kafka_server input using confluent-kafka.
...
def send_and_wait(producer, topic, key, value, headers=None, timeout=10):
    ...
    producer.produce(topic, value=value, key=key, headers=headers, callback=callback)
    producer.flush(timeout=timeout)

    if error[0]:
        raise KafkaException(error[0])
    if not delivered[0]:
        raise Exception("Message not delivered within timeout")
    ...
    try:
        ...
        for i, msg in enumerate(messages):
            send_and_wait(
                ...
                key=f"key-{i}",
                value=msg,
                headers=[("source", b"python-confluent"), ("index", str(i).encode())],
            )
...
def test_sasl_plain_wrong_password():
    ...
    try:
        ...
        send_and_wait(producer, "test-topic", key="test", value=b"should not work")
...
def test_large_message():
    ...
    try:
        ...
        send_and_wait(producer, "large-topic", key="large-key", value=large_data)
