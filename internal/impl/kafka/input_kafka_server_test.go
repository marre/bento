...
// Unit tests for stripInnerLengthPrefix helper
func TestStripInnerLengthPrefix_Int32(t *testing.T) {
	payload := []byte("n,,n=scramuser,r=nonce_inner_int32")
	inner := make([]byte, 4+len(payload))
	binary.BigEndian.PutUint32(inner[0:4], uint32(len(payload)))
	copy(inner[4:], payload)

	out, kind := stripInnerLengthPrefix(inner)
	if kind != "int32" {
		t.Fatalf("expected int32 prefix to be detected, got %q", kind)
	}
	if string(out) != string(payload) {
		t.Fatalf("unexpected output payload: %x", out)
	}
}

func TestStripInnerLengthPrefix_Compact(t *testing.T) {
	payload := []byte("n,,n=scramuser,r=nonce_inner_compact")
	// compact encodes len+1 as uvarint; for small lengths one byte is fine
	prefix := byte(len(payload) + 1)
	inner := append([]byte{prefix}, payload...)

	out, kind := stripInnerLengthPrefix(inner)
	if kind != "compact" {
		t.Fatalf("expected compact prefix to be detected, got %q", kind)
	}
	if string(out) != string(payload) {
		t.Fatalf("unexpected output payload: %x", out)
	}
...
func TestHandleSaslAuthenticate_StripsInnerInt32AndRunsSCRAM(t *testing.T) {
	k := makeSCRAMTestServer(t, "scramuser", "password")

	// Build inner int32 prefix + payload
	payload := []byte("n,,n=scramuser,r=nonce_double")
	inner := make([]byte, 4+len(payload))
	binary.BigEndian.PutUint32(inner[0:4], uint32(len(payload)))
	copy(inner[4:], payload)

	// Wrap inner in top-level Int32 BYTES as the request body would be
	data := make([]byte, 4+len(inner))
	binary.BigEndian.PutUint32(data[0:4], uint32(len(inner)))
	copy(data[4:], inner)
	...
	go func() {
		_, err := k.handleSaslAuthenticate(sConn, 7, 7, data, 2, connState)
		done <- err
	}()
